...
learning_rate = 0.01
epochs_list = [10, 10, 20, 10, 30, 20, 100]
epochs_cumulative = [10, 20 , 40, 50, 80, 100, 200]
mini_batch = 50
...
for epochs in epochs_list:
		...
		for batch in dataset:
			...
			for index,d in enumerate(batch):
				...
				for i in range(len(layers)):
					if i == 0: # set first index of activations as the input feature vectors
						...
						if( not(softmax_bool) or i < len(layers) - 1):
							activation[index][i] = relu(z[index][i - 1])
						else:
							activation[index][i] = softmax(z[index][i - 1])
				# backprop
				# delta = np.multiply(cross_entropy_derivative(activation[index][-1], e), derivative_relu(z[index][-1]))
				...
				for l in xrange(2, len(layers)):
					delta = np.multiply(np.dot(weights[-l+1].transpose(), delta), derivative_relu(z[index][-l]))
	...
	for q in x_test:
		...
		for i in range(len(layers)):
			if i == 0: # set first index of activations as the input feature vectors
				...
				if( not(softmax_bool) or i < len(layers) - 1):
					temp[i] = relu(np.dot(weights[i - 1], temp[i - 1]) + bias[i - 1])
				else:
	...
	accuracy.append(aha)
	print aha
# file = open("weights_large_relu_self.pkl", "wb")
# file1 = open("bias_large_relu_self.pkl", "wb")
